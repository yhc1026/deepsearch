...
请输出 JSON 计划（不要包含任何非 JSON 内容）："""
    ...
    async def _invoke_planner_llm(self, prompt: str) -> str:
        ...
        # 去掉可能的 markdown 围栏
        match = self._JSON_FENCE_PATTERN.search(content)
        if match:
    ...
    def _validate(self, plan: Plan) -> None:
        ...
        if not plan.steps:
            raise PlanValidationError("计划不能为空，至少需要一个步骤")
    ...
    def _check_cycle(self, plan: Plan, step: PlanStep, visited: Optional[set] = None) -> None:
        ...
        for dep_id in step.depends_on:
            ...
            self._check_cycle(plan, dep_step, visited.copy())
...
5. **步骤数量**：简单任务 1-3 步，复杂任务 3-8 步。

6. **知识库优先**：涉及行业分析、市场研究、政策解读时优先用 call_ragflow_query，如果 RAGFlow 返回匹配度过低才考虑 call_network_search 作为后备。
