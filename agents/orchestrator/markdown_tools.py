...
from agents.orchestrator.path_utils import resolve_path
...
def generate_markdown(
    ...
    if path:
        ...
        path_display = "当前会话目录"
    logger.debug(f"输入保存路径: {path_display}")
    ...
    logger.debug(f"当前会话目录: {session_dir}")
    ...
    logger.debug(f"parent_dir={parent_dir}, filename={filename}, full_path={file_path}")

    try:
        # 允许模型指定 session_dir 下的子目录；不存在时自动创建
        if not parent_dir.exists():
            parent_dir.mkdir(parents=True, exist_ok=True)
            logger.debug(f"已创建目录: {parent_dir}")
...
if __name__ == "__main__":
    ...
    if "已成功生成" in result:
        ...
        if file_path.exists():
            ...
            exists_text = "不存在"
        print(f"验证结果：文件 {file_path} {exists_text}")
